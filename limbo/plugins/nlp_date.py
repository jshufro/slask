...
# SOURCE! http://pyparsing.wikispaces.com/UnderDevelopment#toc0
# Special thanks to Paul McGuire of pyparsing. See:
...
def convertToDay(toks):
    ...
    if "wkdayRef" in toks:
        ...
        toks["absTime"] = datetime(now.year, now.month, now.day, now.hour, now.minute)+timedelta(daydiff)
    else:
        name = toks.name.lower()
        toks["absTime"] = {
            "now"       : now,
            "today"     : datetime(now.year, now.month, now.day, now.hour, now.minute),
            "yesterday" : datetime(now.year, now.month, now.day, now.hour, now.minute)+timedelta(-1),
            "tomorrow"  : datetime(now.year, now.month, now.day, now.hour, now.minute)+timedelta(+1),
            }[name]
