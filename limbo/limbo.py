...
def handle_message(event, server):
    ...
    # grep functionality
    if "| grep" in event['text'] and event['text'].startswith('!'):
        try:
            ...
            initial_lines = [ decode(x, 'utf-8') for x in initial_lines]
...
def run_cmd(cmd, server, hook, pluginpath):
    ...
    event = {'type': hook, 'text': cmd, "user": "msguser", 'ts': time.time(),
             'team': None, 'channel': None, 'unfurl_links': True}
